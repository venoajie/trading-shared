...
class UniverseCache:
    ...
    def get_all_instruments(self) -> List[Dict[str, Any]]:
        """Returns the full list of active instruments (Thread-safe copy implied by assignment)."""
    ...
    async def refresh(self):
        ...
        try:
            ...
            for entry in data:
                # Schema Compatibility: Support both 'instrument_name' (Model) and 'symbol' (Legacy/Janitor)
            ...
            log.info(f"UniverseCache refreshed. {len(new_canonical)} mappings. {len(new_list)} instruments.")
