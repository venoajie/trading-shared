...
# Add the project root to sys.path to ensure imports work from the container's WORKDIR
...
async def check_health(service_name: str) -> bool:
    ...
    try:
        ...
        # [FIX] Provide the mandatory 'db' field required by the RedisSettings model.
        settings = RedisSettings(url=redis_url, password=redis_password, db=0)
        ...
        print(f"Healthcheck ERROR: Import failed: {e}. Check PYTHONPATH.", file=sys.stderr)
        return False
    except Exception as e:
        print(f"Healthcheck ERROR: Script exception: {e}", file=sys.stderr)
        return False
...
def main():
    ...
    try:
        ...
        if is_healthy:
            # A silent exit 0 is the docker convention for success
            sys.exit(0)
        else:
            print(f"✗ {service_name} is unhealthy (no heartbeat)", file=sys.stderr)
            sys.exit(1)
    except Exception as e:
        print(f"✗ Healthcheck fatal error: {e}", file=sys.stderr)
        sys.exit(1)
