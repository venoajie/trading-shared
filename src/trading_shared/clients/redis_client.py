...
class CustomRedisClient:
    ...
    async def lrange(self, key: str, start: int, end: int) -> list:
        ...
        return await self.execute_resiliently(lambda pool: pool.lrange(key, start, end), f"LRANGE {key}")

    async def ltrim(self, key: str, start: int, end: int) -> bool:
        """Trim the list at key to the specified range."""
        return await self.execute_resiliently(lambda pool: pool.ltrim(key, start, end), f"LTRIM {key}")
