...
class SystemStateManager:
    ...
    async def can_trade_on_exchange(self, exchange: str) -> bool:
        ...
        """
        global_state = await self.get_global_state()
        if global_state and global_state.status in [SystemState.LOCKED, SystemState.MAINTENANCE]:
        ...
        if exchange_state and exchange_state.status in [SystemState.HEALTHY, SystemState.DEGRADED]:
            return True
